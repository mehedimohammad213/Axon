...
import { transaction, type DbContext } from '../db';
...
async function ensureAdminRole(db: DbContext, organizationId) {
...
async function ensureOrganization(db: DbContext, config: any) {
...
function readFixture(fixtureSubdir: string, file: string): any {
  const filePath = path.join(FIXTURE_ROOT, fixtureSubdir, `${file}.json`);
  if (!fs.existsSync(filePath)) return null;
  return JSON.parse(fs.readFileSync(filePath, 'utf8'));
}

async function fetchJson(url: string, timeoutMs = 120000): Promise<any> {
...
async function loadSourceData(config: any, options: Record<string, unknown> = {}) {
  ...
  if (!options.forceApi && fixtureSubdir) {
    ...
    if (allPresent) {
      ...
      for (const [key, file] of Object.entries(requiredKeys)) {
        data[key] = readFixture(fixtureSubdir, file as string);
      }
...
async function fetchPagesByType(config: any) {
  const pages = [];
...
async function fetchPagesList(config: any) {
...
async function fetchSourceDataFromApi(config: any) {
  if (!config.apiBase) {
...
function saveFixtures(config: any, data: Record<string, unknown>) {
  const fixtureSubdir = config.fixtureSubdir;
  if (!fixtureSubdir) {
...
function createIdMapper(config: any) {
  const idMaps = {
    media: {},
    menuitem: {},
    navbar: {},
...
async function maybeDownloadMediaFile(mediaBase: string | undefined, filePath: string, uploadDir: string) {
...
async function seedOrganizationContent(db: DbContext, organization: any, config: any, data: any) {
...
async function runOrganizationSeeder(db: DbContext, config: any) {
