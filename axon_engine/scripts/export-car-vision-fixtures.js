...
const { extractComponentsFromArray, sortRecursive } = require('../src/utils/helpers');
...
async function upsertByKeys(trx, table, match, payload) {
...
function parseMenuItemIds(menu) {
...
async function loadMenuHeadless(trx, menu) {
...
function buildMainSiteFooterPage(logo, quickMenuHeadless, helpMenuHeadless, platformMenuHeadless, quickMenu, helpMenu, platformMenu) {
  const body = [
    {
...
function buildAboutPage(whyBuyCardModels, whyBuyCardIds) {
...
function buildContactPage(contactCta, contactFormBuilder) {
...
async function upsertPage(trx, organizationId, slug, pageData) {
  const bodyRaw =
    pageData.body_raw ??
 ...
 */
async function seedCarVisionContent(knex, organization) {
